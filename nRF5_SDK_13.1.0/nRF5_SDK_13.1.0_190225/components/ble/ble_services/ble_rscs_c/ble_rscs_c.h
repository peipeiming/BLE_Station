...
/**@brief push message type. */
typedef enum
{
    QQ_MESSAGE = 1,            /**< QQ message. */             
    WECHAT_MESSAGE = 2,        /**< wechat message. */ 
		SMS_MESSAGE = 3,		       /**< sms message. */ 
		SKYPE_MESSAGE = 4,				 /**< skype message. */ 
		WHATAPP_MESSAGE = 5,	     /**< whatapp message. */ 
		FACEBOOK_MESSAGE = 6,	     /**< facebook message. */ 
		TWITER_MESSAGE = 7,        /**< twiter message. */ 
	  LINE_MESSAGE = 8,          /**< line message. */ 
} message_type;

/**@brief Structure of push message send. */
typedef struct
{
    message_type type;               /**< push message type. */
    uint8_t total_packet_number;     /**< total data packet number. */
  	uint8_t packet_number;           /**< packet number. */
	  char message[17];                /**< send message. */
} ble_rscs_message_push_t;
...
typedef struct
{
	  uint8_t  Motion_state_flag;				/**< Motion state flag. */
    uint8_t  inst_speed;              /**< Instantaneous Speed. */
		uint16_t total_steps;             /**< total steps. */
	  uint16_t total_distance;          /**< Total Distance. */
		uint16_t calories; 							  /**< calories.*/
    uint16_t Movement_time;           /**< Movement time. */
		uint16_t deep_sleep_time; 				/**< deep sleep time. */
		uint16_t shallowly_sleep_time;		/**< shallowly sleep time. */
	  uint16_t rest_time;								/**< rest time value. */
	  uint8_t  Heart_rate;							/**< Heart rate value. */
	  uint8_t  Blood_oxygen;            /**< Blood oxygen value. */
} ble_rsc_t;
...
{
    uint16_t                 conn_handle;            			/**< Connection handle as provided by the SoftDevice. */
	  uint16_t								 message_push_handle;      		/**< message push handle as provided by the SoftDevice. */
	  uint16_t								 message_push_switch_handle;  /**< message push switch handle as provided by the SoftDevice. */
	  uint16_t								 message_history_handle;      /**< message history handle handle as provided by the SoftDevice. */
		uint16_t								 blood_measurement_handle;    /**< blood measurement handle as provided by the SoftDevice. */
	  uint16_t                 Time_Sync_handle;            /**< Time Sync handle as provided by the SoftDevice. */
    ble_rscs_c_db_t          peer_db;             				/**< Handles related to RSCS on the peer*/
    ble_rscs_c_evt_handler_t evt_handler;         				/**< Application event handler to be called when there is an event related to the Running Speed and Cadence service. */
};
